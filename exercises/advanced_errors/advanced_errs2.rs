...
// the compiler is complaining about no std::error::Error for ParseClimateError so...
impl Error for ParseClimateError {} // now there is an 'implementation' for parseclimateerror. we do not need changes??

// The `Display` trait allows for other code to obtain the error formatted
// as a user-visible string.

// implementing display infers ToString and thus to_string().
...
impl FromStr for Climate {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let (city, year, temp) = match v[..] {      // borrow not needed - v can end?
            [city, year, temp] => {
                if city.len() != 0 {
        ...
        let year: u32 = year.parse()?; // as we have implemented parsefloaterror, we will ::from to that.
